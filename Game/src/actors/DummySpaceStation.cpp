...
void DummySpaceStation::Initialize()
{
	Actor::Initialize();
...
void DummySpaceStation::UpdateHealth()
{
	...
	if (level)
	{
		...
		if (hud)
		{
			hud->OnSpaceStationHealthUpdated(m_CurrentHealth, m_MaxHealth);
		}
	}
}

void DummySpaceStation::OnTakeDamage(float damage, Crystal::Weak<Actor> causer)
{
