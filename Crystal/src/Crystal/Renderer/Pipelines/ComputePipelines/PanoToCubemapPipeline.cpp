...
namespace Crystal {
	...
	void PanoToCubemapPipeline::Begin(const PipelineInputs* const pipelineInputs)
	{
		...
		D3D12_CPU_DESCRIPTOR_HANDLE handle = m_DescriptorHeap->GetCPUDescriptorHandleForHeapStart();
		device->CopyDescriptorsSimple(1, handle, resourceManager.GetTexture("Pano_Skybox_Space").lock()->GetShaderResourceView(),
			D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
		handle.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

		device->CopyDescriptorsSimple(1, handle, resourceManager.GetTexture("Cube_Skybox_Space").lock()->GetUnorderedAccessView(),
			D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
