...
namespace Crystal {
	...
	StaticSubMesh::StaticSubMesh(aiMesh* mesh) : SubMesh()
	{
	...
	void Mesh::processNode(aiNode* rootNode, const aiScene* scene, bool bIsSkeletal /*= false*/)
	{
		...
		{
			processNode(rootNode->mChildren[i], scene, bIsSkeletal);
		}
	}
	...
	StaticMesh::StaticMesh(const std::string& filePath) : Mesh(filePath)
	{
		...
		{
			m_VertexBuffers.push_back(std::make_unique<VertexBuffer>(((StaticSubMesh*)(submesh.get()))->StaticVertices.data(), (UINT)sizeof(StaticVertex), 
	...
	SkeletalMesh::SkeletalMesh(const std::string& meshFilePath, const std::string& animationFilePath /*= ""*/) : Mesh(meshFilePath)
	{
		...
		processNode(m_MeshScene->mRootNode, m_MeshScene, true);
		...
		if (bHasSeperateFileForAnimation)
		{
			...
			CS_FATAL(!(!m_AnimationScene || !m_AnimationScene->mRootNode), "%s 애니메이션 데이터를 로드하는데 실패하였습니다", animationFilePath.c_str());
		}
		...
		for (const auto& submesh : m_Submeshes)
		{
			m_VertexBuffers.push_back(std::make_unique<VertexBuffer>(((SkeletalSubMesh*)(submesh.get()))->SkeletalVertices.data(), (UINT)sizeof(SkeletalVertex), 
	...
	void SkeletalMesh::Update(float deltaTime)
	{
		...
		float ticksPerSecond = m_AnimationScene->mAnimations[0]->mTicksPerSecond != 0 ? (float)m_AnimationScene->mAnimations[0]->mTicksPerSecond : 25.0f;
