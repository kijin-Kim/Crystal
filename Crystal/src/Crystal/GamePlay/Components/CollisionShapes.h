...
namespace boost {
	namespace serialization {
		...
		void serialize(Archive& ar, Crystal::Collision::BoundingSphere& g, const unsigned int version)
		{
			ar& g.Center; // Center of the sphere.
			ar& g.Radius; // Radius of the sphere.
		}

		template <class Archive>
		void serialize(Archive& ar, Crystal::Collision::BoundingBox& g, const unsigned int version)
		{
			ar& g.Center; // Center of the box.
			ar& g.Extents; // Distance from the center to each side.
		}


		template <class Archive>
		void serialize(Archive& ar, Crystal::Collision::BoundingOrientedBox& g, const unsigned int version)
		{
			ar& g.Center; // Center of the box.
			ar& g.Extents; // Distance from the center to each side.
			ar& g.Orientation; // Unit quaternion representing rotation (box -> world).
		}
